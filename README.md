...
```bash
python3 scripts/agent_blogger.py init-config --output agent-blogger.config.yaml
python3 scripts/agent_blogger.py inspect /path/to/session.jsonl
python3 scripts/agent_blogger.py reduce /path/to/session.jsonl --config agent-blogger.config.yaml --output issue-context.json
...
python3 scripts/agent_blogger.py pipeline /path/to/session.jsonl --config agent-blogger.config.yaml --output /path/to/blog/source/_posts/example.md
```
...
```text
命令行 `--source-path` 或位置参数 > 配置 `source.path` > Agent 当前会话
```

OpenClaw 场景可以通过 `--session-key` 或 `source.session_key` 指定会话；相对路径会按 `--base-dir` 或 `source.base_dir` 解析。
